...
class LinearAllocator : public Allocator {
 public:
  LinearAllocator(std::size_t size)
      : _size(size), _ptr(new char[size]), _offset(0) {}

  ~LinearAllocator() override { delete[] _ptr; }
...
class StackAllocator : public Allocator {
 public:
  StackAllocator(std::size_t size)
      : _size(size), _ptr(new char[size]), _top(0) {}

  ~StackAllocator() override { delete[] _ptr; }
  ...
  void *Allocate(std::size_t size, std::size_t typeSize) override {
    ...
    AllocationHeader *header =
        reinterpret_cast<AllocationHeader *>(allocationPtr);
...
class FreeListAllocator : public Allocator {
 public:
  FreeListAllocator(std::size_t size)
      : _size(size), _ptr(new char[size]), _freeList(_ptr) {
    FreeBlock *firstBlock = reinterpret_cast<FreeBlock *>(_freeList);
    firstBlock->size = _size;
    firstBlock->next = nullptr;
  }

  ~FreeListAllocator() override { delete[] _ptr; }
  ...
  void *Allocate(std::size_t count, std::size_t typeSize) override {
    ...
    while (current) {
      if (current->size >= count * typeSize) {
        if (current->size > count * typeSize) {
          char *newBlockPtr =
              reinterpret_cast<char *>(current) + count * typeSize;
          FreeBlock *newBlockHeader =
              reinterpret_cast<FreeBlock *>(newBlockPtr);
...
class PoolAllocator : public Allocator {
 public:
  PoolAllocator(std::size_t blockCount, std::size_t blockSize)
      : _blockSize(blockSize), _blockCount(blockCount) {
    _pool = new char[blockSize * blockCount];
    InitializePoolList();
  }

  ~PoolAllocator() override { delete[] _pool; }
...
class StandardAllocator {
 public:
  ...
  StandardAllocator &operator=(const StandardAllocator &other) noexcept =
      default;
...
constexpr bool operator==(const StandardAllocator<T> &,
                          const StandardAllocator<U> &) noexcept {
  return true;
}
...
template <typename T>
StandardAllocator<T>::StandardAllocator(Allocator &allocator)
